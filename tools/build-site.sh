...
DIST="$ROOT/dist"
CLIENT="$DIST/client"
SOCIAL_CARD="$ROOT/assets/media/social/modscan-social-card.png"
...
if [ -f "$SOCIAL_CARD" ]; then
  cp "$SOCIAL_CARD" "$CLIENT/assets/media/social/modscan-social-card.png"
fi

cp "$ROOT/tools/sites-static-worker.js" "$DIST/server/index.js"

printf 'Built static ModScan site in %s\n' "$DIST"
